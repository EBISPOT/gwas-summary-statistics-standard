...
\newenvironment{tablenotes}{\begin{list}{}{\setlength{\leftmargin}{1em}\setlength{\itemsep}{0pt}\small}}{\end{list}}

\title{GWAS-SSF: A GWAS Summary Statistics Format (v1.1.0)}
\date{\today}
...
\item A set of mandatory (i.e.\ must be present and, except where this specification explicitly permits \texttt{\#NA}, filled with non-null values) fields, providing the information necessary to enable a wide range of data analyses including MR and PGS development
...
\begin{landscape}
\subsubsection{Z-score fallback example file data}
\scriptsize
\begin{verbatim}
chromosome  base_pair_location  effect_allele  other_allele  z-score    standard_error  effect_allele_frequency  p_value  variant_id       rsid         ref_allele
1           869388              A              G             -2.0619    #NA             0.997221                 0.0392   1_869388_A_G     #NA          EA
1           205813916           G              C             -2.6992    #NA             0.983589                 0.00695  1_205813916_G_C  rs74143855   EA
2           70478797            T              TG            11.1833    #NA             0.934121                 3.5E-30  2_70478797_T_TG  rs142640435  EA
7           8458030             TC             T             -18.2089   #NA             0.78451                  5.7E-76  7_8458030_TC_T   rs774624811  EA
23          24173186            A              C             0.0443     #NA             0.627178                 0.9647   23_24173186_C_A  rs5949233    OA
\end{verbatim}

\normalsize
        This example shows the fallback representation for summary statistics where the source analysis only provides a z-score and does not provide an effect estimate with an uncertainty estimate. In this case the effect statistic column is labelled \textit{z-score}. The \textit{standard\_error} column remains present at column 5, preserving the standard column order, and every value in that column is the literal missing value \textit{\#NA}. This fallback should only be used when \textit{beta}, \textit{odds\_ratio} and \textit{hazard\_ratio} are unavailable.
\end{landscape}
% ----------
...
\begin{table}[h]
 \begin{threeparttable}
  ...
  \begin{tabularx}{\linewidth} { 
   ...
   \textit{beta} & Column 4: Effect size as beta & Numeric & Mandatory that either \textit{beta}, \textit{odds\_ratio}, \textit{hazard\_ratio} or the \textit{z-score} fallback is given\\
   ...
   \textit{z-score} & Column 4: Z-score fallback statistic & Numeric & Fallback only where \textit{beta}, \textit{odds\_ratio} and \textit{hazard\_ratio} are unavailable\\
   \textit{standard\_error} & Column 5: Standard error & Numeric, except \texttt{\#NA} for z-score fallback files\tnote{d} & Mandatory\\
  ...
  \end{tabularx}
   \begin{tablenotes}
    ...
    \item [d] \textit{standard\_error} may only contain \texttt{\#NA} when \textit{z-score} is used as the column 4 fallback statistic. Otherwise \textit{standard\_error} must be numeric.
   \end{tablenotes}
...
\begin{table}[h]
 ...
 \begin{tabularx}{\textwidth} { 
  ...
  \textit{sex} & Indicate if and how the study was sex-stratified & ``M'', ``F'', ``combined'', ``NR'' & No\\
  \hline
 \end{tabularx}
...
\subsubsection{Z-score fallback metadata example}

% Z-SCORE FALLBACK METADATA EXAMPLE
\verbatiminput{./examples/0000124-z-score.tsv-meta.yaml}
% ----------
